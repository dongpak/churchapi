...
public class ChurchContactResourceSpec implements Specification<ChurchContactEntity> {
     ...
     * @param criteria
     */
    public ChurchContactResourceSpec(Church criteria) {
        this.criteria = criteria;
    }

    @Override
    public Predicate toPredicate(Root<ChurchContactEntity> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
    ...
    private void addPredicate(CriteriaBuilder criteriaBuilder, Root<ChurchContactEntity> root, String field, String value, List<Predicate> predicates) {
